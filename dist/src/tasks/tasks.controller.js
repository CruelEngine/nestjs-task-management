...
let TasksController = class TasksController {
    ...
    getAllTasks(filterDto, user) {
        this.logger.log(`User ${user} retrieving all tasks. Filters : ${JSON.stringify(filterDto)}`);
    ...
    createTask(createTaskDto, user) {
        this.logger.log(`USer : ${user.username} creating a task . Data : ${JSON.stringify(createTaskDto)}`);
        return this.tasksService.createTask(createTaskDto, user);
    }
    deleteTask(id, user) {
        this.logger.log(`USer : ${user.username} delete a task . Data : ${JSON.stringify(id)}`);
...
__decorate([
    ...
    __param(1, common_1.Body('status', task_status_validation_pipe_1.TaskStatusValidationPipe)), __param(2, get_user_decorator_1.GetUser()),
    __metadata("design:type", Function),
